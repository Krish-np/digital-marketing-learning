<div align="center">

# Digital Marketing Playbook

![Stars](https://img.shields.io/github/stars/Krish-np/digital-marketing-learning?style=for-the-badge&logo=github&logoColor=white&color=FFD700)
![Forks](https://img.shields.io/github/forks/Krish-np/digital-marketing-learning?style=for-the-badge&logo=github&logoColor=white&color=6A5ACD)
![License](https://img.shields.io/github/license/Krish-np/digital-marketing-learning?style=for-the-badge&color=22C55E)
![Last Commit](https://img.shields.io/github/last-commit/Krish-np/digital-marketing-learning?style=for-the-badge&logo=git&logoColor=white&color=F97316)
![Repo Size](https://img.shields.io/github/repo-size/Krish-np/digital-marketing-learning?style=for-the-badge&color=3B82F6)
![Issues](https://img.shields.io/github/issues/Krish-np/digital-marketing-learning?style=for-the-badge&color=EF4444)
![Markdown](https://img.shields.io/badge/Built%20With-Markdown-blue?style=for-the-badge&logo=markdown&logoColor=white&color=0EA5E9)
![Beginner Friendly](https://img.shields.io/badge/Beginner-Friendly-success?style=for-the-badge&color=10B981)

> A comprehensive beginner-friendly Digital Marketing learning repository containing structured notes, practical guides, real-world examples, checklists, and mini-projects from my college Digital Marketing course.

</div>

---

## Table of Contents

- [Repository Overview](#repository-overview)
- [Repository Structure](#repository-structure)
- [Topics Covered](#topics-covered)
- [Learning Roadmap](#learning-roadmap)
- [Skills You'll Learn](#skills-youll-learn)
- [Practical Projects](#practical-projects)
- [Repository Highlights](#repository-highlights)
- [Tools Used](#tools-used)
- [How to Use This Repository](#how-to-use-this-repository)
- [Folder Guide](#folder-guide)
- [Current Best Practices](#current-best-practices)
- [Recommended Learning Order](#recommended-learning-order)
- [Future Improvements](#future-improvements)
- [Contributing](#contributing)
- [License](#license)
- [Author](#author)

---

## Repository Overview

This repository is designed to become a complete learning resource for anyone interested in Digital Marketing.

Instead of simply storing class notes, this project organizes concepts into structured documentation with practical examples, implementation guides, best practices, and practical exercises.

Whether you're a:

- Student
- Beginner
- Freelancer
- WordPress Developer
- Small Business Owner
- Job Seeker

...this repository provides a solid foundation for learning modern Digital Marketing.

---

## Repository Structure

```text
digital-marketing-learning/
│
├── README.md
├── LICENSE
│
├── assets/
│   ├── images/
│   ├── screenshots/
│   └── diagrams/
│
├── 01-digital-marketing-fundamentals/
├── 02-seo/
├── 03-wordpress/
├── 04-yoast-seo/
├── 05-social-media-marketing/
├── 06-meta-ads/
├── 07-google-tools/
├── 08-email-marketing/
├── 09-content-marketing/
├── 10-case-studies/
├── 11-checklists/
└── resources/
```

---

## Topics Covered

### 1. Digital Marketing Fundamentals

- What is Digital Marketing?
- Marketing Funnel
- Customer Journey
- Buyer Persona
- AIDA Model
- Digital Marketing Channels
- Digital Marketing Strategy

### 2. Search Engine Optimization (SEO)

- Search Engines
- Keyword Research
- Search Intent
- On-page SEO
- Off-page SEO
- Technical SEO
- Local SEO
- Backlinks
- Internal Linking
- SEO Audit
- Robots.txt
- XML Sitemap

### 3. WordPress

- Installation
- Dashboard
- Posts & Pages
- Categories & Tags
- Themes & Plugins
- Widgets & Menus
- Security
- Performance Optimization

### 4. Yoast SEO

- Installation & Configuration
- Focus Keyphrase
- SEO & Readability Analysis
- Meta Title & Description
- Canonical URL
- XML Sitemap
- Schema
- Social Preview
- Common Mistakes

### 5. Social Media Marketing

- Facebook Marketing
- Instagram Marketing
- LinkedIn Marketing
- YouTube Marketing
- X (Twitter)
- Content Planning
- Organic Reach
- Engagement Strategy
- Hashtag Research

### 6. Meta Ads

- Meta Business Suite
- Business Manager
- Ad Account Setup
- Campaign Objectives
- Ad Sets & Audience Targeting
- Budgeting & Placements
- Pixel Basics
- Campaign Optimization
- Performance Metrics

### 7. Google Tools

- Google Analytics 4
- Google Search Console
- Google Trends
- Google Keyword Planner
- Google Tag Manager (Basics)

### 8. Email Marketing

- Newsletter Basics
- Lead Magnets
- Email Campaigns
- Open Rate & Click Through Rate
- Email Best Practices

### 9. Content Marketing

- Blogging
- SEO Writing
- Copywriting
- Keyword Placement
- Content Calendar
- Calls to Action (CTA)

### 10. Practical Case Studies

- WordPress SEO Optimization
- Installing Yoast SEO
- Meta Ads Campaign Setup
- Facebook & Instagram Integration
- Google Analytics Integration
- Search Console Verification
- Publishing an SEO-Optimized Blog Post

### 11. Practical Checklists

- SEO Audit Checklist
- WordPress Launch Checklist
- Blog Publishing Checklist
- Website Optimization Checklist
- Meta Ads Checklist

---

## Learning Roadmap

```text
Digital Marketing Fundamentals
            │
            ▼
        SEO Basics
            │
            ▼
       WordPress
            │
            ▼
        Yoast SEO
            │
            ▼
 Social Media Marketing
            │
            ▼
        Meta Ads
            │
            ▼
      Google Analytics
            │
            ▼
     Content Marketing
            │
            ▼
      Email Marketing
            │
            ▼
     Practical Projects
```

---

## Skills You'll Learn

After completing this repository, you should be able to:

- Understand Digital Marketing fundamentals
- Perform keyword research
- Optimize WordPress websites
- Configure Yoast SEO
- Improve website SEO
- Build content strategies
- Create Meta advertising campaigns
- Connect Meta Business tools
- Configure Google Analytics 4
- Set up Google Search Console
- Monitor website traffic
- Interpret marketing metrics
- Publish SEO-friendly blog posts
- Conduct basic SEO audits

---

## Practical Projects

This repository includes step-by-step documentation for:

- SEO optimization of a WordPress website
- Installing and configuring Yoast SEO
- Creating a Meta Ads campaign
- Connecting Facebook and Instagram
- Connecting Google Analytics 4
- Connecting Google Search Console
- Publishing an SEO-optimized blog post
- Performing a complete website SEO audit

---

## Repository Highlights

✔ Beginner Friendly  
✔ Step-by-Step Tutorials  
✔ Real-World Examples  
✔ Practical Exercises  
✔ Mini Projects  
✔ Best Practices  
✔ Common Mistakes  
✔ Visual Diagrams  
✔ Professional Documentation  
✔ GitHub Portfolio Project  

---

## Tools Used

| Category         | Tools                  |
| ---------------- | ---------------------- |
| CMS              | WordPress              |
| SEO              | Yoast SEO              |
| Analytics        | Google Analytics 4     |
| Search           | Google Search Console  |
| Keyword Research | Google Keyword Planner |
| Trends           | Google Trends          |
| Advertising      | Meta Ads Manager       |
| Social           | Meta Business Suite    |
| Documentation    | Markdown               |
| Version Control  | Git & GitHub           |

---

## How to Use This Repository

If you're new to Digital Marketing:

1. Start with **Digital Marketing Fundamentals**
2. Learn the basics of SEO
3. Move on to WordPress
4. Practice Yoast SEO
5. Explore Social Media Marketing
6. Learn Meta Ads
7. Set up Google Analytics and Search Console
8. Complete the practical projects
9. Use the checklists before launching any website

---

## Folder Guide

| Folder      | Description                          |
| ----------- | ------------------------------------ |
| `01`        | Digital Marketing Fundamentals       |
| `02`        | SEO                                  |
| `03`        | WordPress                            |
| `04`        | Yoast SEO                            |
| `05`        | Social Media Marketing               |
| `06`        | Meta Ads                             |
| `07`        | Google Tools                         |
| `08`        | Email Marketing                      |
| `09`        | Content Marketing                    |
| `10`        | Case Studies                         |
| `11`        | Checklists                           |
| `resources` | Books, blogs, tools, and references  |

---

## Current Best Practices

Digital Marketing changes rapidly. Throughout this repository, you'll find notes where modern practices differ from traditional college material.

Examples include:

- GA4 replacing Universal Analytics
- Increasing importance of search intent over keyword density
- AI-assisted content creation with human review
- Core Web Vitals as SEO ranking considerations
- First-party data becoming more important due to privacy changes

Whenever a topic has evolved, the documentation will explain both the traditional concept and the current recommended approach.

---

## Recommended Learning Order

1. Digital Marketing Fundamentals
2. SEO
3. WordPress
4. Yoast SEO
5. Content Marketing
6. Social Media Marketing
7. Meta Ads
8. Google Tools
9. Email Marketing
10. Case Studies
11. Checklists

---

## Future Improvements

Planned additions include:

- Google Ads (Search & Display)
- Technical SEO audits with real websites
- AI tools for Digital Marketing
- Conversion Rate Optimization (CRO)
- Heatmap tools
- Marketing automation
- UTM campaign builder examples
- Local SEO case studies
- Landing page optimization
- Marketing interview questions

---

## Contributing

Contributions are welcome!

You can help by:

- Fixing errors
- Updating outdated information
- Adding practical examples
- Improving documentation
- Suggesting better resources

Please open an Issue before submitting significant changes.

---

## License

This project is licensed under the [MIT License](./LICENSE).

---

## Author

**Krishna Thapa**

- Full-Stack Developer
- WordPress Developer
- SEO Enthusiast
- Digital Marketing Learner

[![GitHub](https://img.shields.io/badge/GitHub-Krish--np-181717?style=flat-square&logo=github)](https://github.com/Krish-np)
[![Portfolio](https://img.shields.io/badge/Portfolio-krishnathapa.name.np-blue?style=flat-square)](https://krishnathapa.name.np)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Krishna%20Thapa-0077B5?style=flat-square&logo=linkedin)](https://www.linkedin.com/in/krishna-thapa-a66477318/)

---

> If this repository helps you learn something new, consider giving it a ⭐ to support the project.
>
> Happy Learning!
